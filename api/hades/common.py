...
class FinancesSettings:
    ...
    def top_up_cash_reserve_from_daily_expense_reserve_account(wise_account: WiseAccount | None = None) -> None:
        ...
        if amount_within_budget < Decimal("0"):
            ...
            asyncio.ensure_future(Discord.notify(f"{message_title}"
                                                 f"*Amount over budget*: {reserve_account.currency.value}{common.get_decimal_str(amount_out_of_budget)}\n"
                                                 f"*Date when under budget*: {common.get_date_string(catch_up_date)} ({number_of_days_to_catch_up} days)"))
        else:
            asyncio.ensure_future(Discord.notify(f"{message_title}"
                                                 f"*Amount within budget*: {reserve_account.currency.value}{common.get_decimal_str(amount_within_budget)}"))

    @staticmethod
    def _get_daily_budget(wise_account: WiseAccount, daily_expense: Optional["PlannedExpense"] = None, date: datetime.date | None = None) -> Decimal:
        daily_expense = PlannedExpense.get(PlannedExpenseNeed.DAILY_EXPENSES, wise_account) if daily_expense is None else daily_expense
...
class PlannedExpense(DataClass):
    ...
    @staticmethod
    def do_scheduled_planned_expense(planned_expense: "PlannedExpense") -> None:
        nzd_account: CashAccount = planned_expense.reserve_account.profile.get_cash_account(Currency.NZD)
        planned_expense.reserve_account.transfer(nzd_account, planned_expense.amount)
        asyncio.ensure_future(sms.send_message(planned_expense.notification_phone_number, f"{planned_expense.reserve_account.currency.value}{common.get_decimal_str(planned_expense.amount)} has been transferred to your account.\n"
    ...
    def get_all(wise_account: WiseAccount | None = None, excel_file_path: str | None = None) -> Tuple[List["PlannedExpense"], List["PlannedExpense"], List["PlannedExpense"], "PlannedExpense"]:
        ...
        wise_account = WiseAccount.get(WiseAccountType.PRIMARY) if wise_account is None else wise_account
        needs_planned_expense_list, wants_planned_expense_list = PlannedExpense._get_needs_and_wants_list(wise_account, excel_file_path)
        scheduled_planned_expense_list: List[PlannedExpense] = PlannedExpense._get_scheduled_list(wise_account, excel_file_path)
    ...
    def get(planned_expense_title: PlannedExpenseTitle, wise_account: WiseAccount | None = None) -> "PlannedExpense":
        ...
        if planned_expense_type == planned_expense_type.NEEDS:
            ...
            return next(filter(lambda e: e.reserve_account.name == reserve_account_name, scheduled_planned_expense_list))
    ...
    @staticmethod
    def _get_needs_and_wants_list(wise_account: WiseAccount, excel_file_path: str | None = None) -> Tuple[List["PlannedExpense"], List["PlannedExpense"]]:
        excel_file_path = FinancesSettings.get_monthly_finances_excel_file_path() if excel_file_path is None else excel_file_path
        ...
        for planned_expense_type in [PlannedExpenseType.NEEDS, PlannedExpenseType.WANTS]:
            raw_planned_expense_list: List[Dict[Any, Any]] = excel.get_excel_data(excel_file_path, planned_expense_type.value)
            for raw_planned_expense in raw_planned_expense_list:
                ...
                planned_expense: PlannedExpense = PlannedExpense(
                    ...
                    reserve_account=wise_account.personal_profile.get_reserve_account(f"{description}{PlannedExpense._get_reserve_account_suffix(planned_expense_type)}", currency, True)
                )
    ...
    def _get_scheduled_list(wise_account: WiseAccount, excel_file_path: str | None = None) -> List["PlannedExpense"]:
        excel_file_path = FinancesSettings.get_monthly_finances_excel_file_path() if excel_file_path is None else excel_file_path
        raw_scheduled_planned_expense_list: List[Dict[Any, Any]] = excel.get_excel_data(excel_file_path, "Scheduled Transfers")
        planned_expense_list: List[PlannedExpense] = []
    ...
    @staticmethod
    def _get_savings_amount(planned_expense_list: List["PlannedExpense"], salary: Decimal, wise_account: WiseAccount) -> Decimal:
        ...
        for planned_expense in planned_expense_list:
            ...
            if to_account.currency == nzd_balance.currency:
                ...
                quote: Quote = Quote.get_quote(wise_account.personal_profile, nzd_balance, to_account, planned_expense.amount)
